...
/mob/living/simple_animal/bot/nutribot/proc/get_controls(mob/user)
	...
	if(!locked || issilicon(user) || IsAdminGhost(user))
		...
		dat += "Hunger Check: "
		dat += "<a href='byond://?src=[REF(src)];hunger_check=1'>[(hunger_check==NUTRITION_LEVEL_HUNGRY) ? "Yes" : "No"]</a><br>"

		dat += "<TT>Maximum BFI: "
		dat += "[feed_threshold ? feed_threshold : "???"] "
		dat += "<br>"
		dat += "Disable Maximum BFI: <a href='byond://?src=[REF(src)];feed_threshold=1'>["Disable"]</a><br> "
		dat += "<a href='byond://?src=[REF(src)];feed_threshold=-100'>-</a> "
		dat += "<a href='byond://?src=[REF(src)];feed_threshold=100'>+</a> "
		dat += "</TT><br>"
...
/mob/living/simple_animal/bot/nutribot/process_scan(mob/living/carbon/human/H)
	...
	if(assess_patient(H))
		...
		if((last_newpatient_speak + 300) < world.time) //Don't spam these messages!
			...
			if(prob(2) && ISINRANGE_EX(H.get_fire_loss(), 0, 20))
